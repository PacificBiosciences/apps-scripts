...
def waterfallPlot2(data,row=None,x='position',y='idx',hue='motif'):
    ...
    g = sns.FacetGrid(data=data,
                      ...
                      aspect=2,height=3,
                      sharex=False,sharey=False)
    for n in range(motifLengths.min()):
...
def countPlot(data,targetDict):
    ...
    if 'target' not in data.columns:
        data['target'] = list(targetDict.keys())[0]
    #filter for primary motif
    qry    = '|'.join([f'(target=="{t}" & motif=="{ms.split(",")[0]}")'
                       for t,ms in list(targetDict.items())])
...
def countPlot2(counts,target,xlabel,binsize=1):
    ...
    ax.hist(counts,
            bins=range(min(counts)-1,max(counts)+1,binsize),
            align='left',
            alpha=0.5)
